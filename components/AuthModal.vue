        ...
        <div class="modal-content" @click.stop>
          <div class="modal-header">
            <h3 class="modal-title">
              {{ user ? 'Perfil de usuario' : (isSignUp ? 'Crear cuenta' : 'Iniciar sesión') }}
            </h3>
            ...
            <div v-if="user" class="profile-section">
              <div class="profile-info">
                <div class="profile-avatar">
                  <img v-if="user.photoURL" :src="user.photoURL" alt="Avatar" class="avatar-image">
                  <div v-else class="avatar-placeholder">
                    {{ (user.displayName || user.email || 'U').charAt(0).toUpperCase() }}
                  </div>
                </div>
                <div class="profile-details">
                  <h3 class="profile-name">{{ user.displayName || 'Usuario' }}</h3>
...
const { user, signInWithGoogle, signInWithEmail, signUpWithEmail, signOut, loading: authLoading, error: authError, clearError } = useAuth()
...
const updateUserProfile = async () => {
  ...
  try {
    await updateProfile(user.value as any, { displayName: newDisplayName.value.trim() })
...
const resetPassword = async () => {
  if (!email.value) {
    profileError.value = 'Ingresa tu email para resetear la contraseña'
    return
  }
  profileError.value = null
  try {
    await sendPasswordResetEmail(getAuth(), email.value)
    profileError.value = 'Email de reseteo enviado'
  } catch (err: any) {
    profileError.value = err.message
  }
}
...
.profile-avatar {
  @apply w-16 h-16 rounded-full overflow-hidden bg-orange-500 flex items-center justify-center;
}
...
.update-button {
  @apply w-full py-2 px-4 bg-orange-500 text-white font-semibold rounded-lg hover:bg-orange-600 transition-colors;
}
...
.reset-button {
  @apply flex-1 py-2 px-4 bg-gray-500 text-white font-semibold rounded-lg hover:bg-gray-600 transition-colors;
}

.logout-button {
  @apply flex-1 py-2 px-4 bg-red-500 text-white font-semibold rounded-lg hover:bg-red-600 transition-colors;
}
